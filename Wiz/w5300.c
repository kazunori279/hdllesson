...
uint8 getSn_SSR(SOCKET s) {
	...
	while (1) {
		...
		if (ssr == ssr1)
			break; // if first == sencond, Sn_SSR value is valid.
...
uint32 getSn_RX_RSR(SOCKET s) {
	...
	while (1) {
		...
		received_rx_size = (received_rx_size << 16)
				+ IINCHIP_READ(Sn_RX_RSR2(s)); // read                                       
...
uint8 sysinit(uint8* tx_size, uint8* rx_size) {
	...
	for (i = 0; i < MAX_SOCK_NUM; i++) {
		if (tx_size[i] > 64) {
			...
			alt_printf("Illegal Channel(%d) TX Memory Size.\r\n", i);
#endif
			return 0;
		}
		if (rx_size[i] > 64) {
#ifdef __DEF_IINCHIP_DBG__         
			alt_printf("Illegal Channel(%d) RX Memory Size.\r\n", i);
#endif
			return 0;
		}
...
#ifdef __DEF_IINCHIP_DBG__
		alt_printf("Illegal Memory Allocation\r\n");
#endif
		return 0;
	}
...
#ifdef __DEF_IINCHIP_DBG__
	alt_printf("Total TX Memory Size = %dKB\r\n", ssum);
	alt_printf("Total RX Memory Size = %dKB\r\n", rsum);
	alt_printf("Ch : TX SIZE : RECV SIZE\r\n");
	alt_printf("%02d : %07dKB : %07dKB \r\n", 0,
			(uint8) (IINCHIP_READ(TMS01R) >> 8),
			(uint8) (IINCHIP_READ(RMS01R) >> 8));
	alt_printf("%02d : %07dKB : %07dKB \r\n", 1, (uint8) IINCHIP_READ(TMS01R),
			(uint8) IINCHIP_READ(RMS01R));
	alt_printf("%02d : %07dKB : %07dKB \r\n", 2,
			(uint8) (IINCHIP_READ(TMS23R) >> 8),
			(uint8) (IINCHIP_READ(RMS23R) >> 8));
	alt_printf("%02d : %07dKB : %07dKB \r\n", 3, (uint8) IINCHIP_READ(TMS23R),
			(uint8) IINCHIP_READ(RMS23R));
	alt_printf("%02d : %07dKB : %07dKB \r\n", 4,
			(uint8) (IINCHIP_READ(TMS45R) >> 8),
			(uint8) (IINCHIP_READ(RMS45R) >> 8));
	alt_printf("%02d : %07dKB : %07dKB \r\n", 5, (uint8) IINCHIP_READ(TMS45R),
			(uint8) IINCHIP_READ(RMS45R));
	alt_printf("%02d : %07dKB : %07dKB \r\n", 6,
			(uint8) (IINCHIP_READ(TMS67R) >> 8),
			(uint8) (IINCHIP_READ(RMS67R) >> 8));
	alt_printf("%02d : %07dKB : %07dKB \r\n", 7, (uint8) IINCHIP_READ(TMS67R),
			(uint8) IINCHIP_READ(RMS67R));
	alt_printf("\r\nMTYPER=%04x\r\n", IINCHIP_READ(MTYPER));
#endif

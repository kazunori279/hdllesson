...
uint8 socket(SOCKET s, uint8 protocol, uint16 port, uint16 flag) {
	...
	alt_printf("%x : Sn_MR=0x%x,Sn_PORTR=0x%x(%x),Sn_SSR=%x\r\n", s,
			IINCHIP_READ(Sn_MR(s)), IINCHIP_READ(Sn_PORTR(s)),
			IINCHIP_READ(Sn_PORTR(s)), getSn_SSR(s));
#endif
	return 1;
}
...
void close(SOCKET s) {
	...
	if (((getSn_MR(s) & 0x0F) == Sn_MR_TCP)
			&& (getSn_TX_FSR(s) != getIINCHIP_TxMAX(s))) {
		uint16 loop_cnt = 0;
...
#ifdef __DEF_IINCHIP_DBG__
		alt_printf("%x : Fail[invalid ip,port]\r\n", s);
#endif
		return 0;
	}
...
#ifdef __DEF_IINCHIP_DBG__
		alt_printf("%x : SOCKET is not created!\r\n", s);
#endif
		return 0;
	}
...
uint32 send(SOCKET s, uint8 * buf, uint32 len) {
	...
	do {
		...
		status = getSn_SSR(s);
#ifdef __DEF_IINCHIP_DBG__
		alt_printf("%x : freesize=%ld\r\n", s, freesize);
		if (loopcnt++ > 0x0010000) {
			alt_printf("%x : freesize=%ld,status=%04x\r\n", s, freesize,
					status);
			alt_printf("%x:Send Size=%08lx(%x)\r\n", s, ret, ret);
			alt_printf("MR=%04x\r\n", *((vuint16*) MR));
			loopcnt = 0;
		}
#endif
	...
	if (!check_sendok_flag[s]) // if first send, skip.
	{
		while (!(getSn_IR(s) & Sn_IR_SENDOK)) // wait previous SEND command completion.
		{
			...
			{
#ifdef __DEF_IINCHIP_DBG__
				alt_printf("%x : Send Fail. SOCK_CLOSED.\r\n", s);
#endif
				return 0;
			}
		}
...
uint32 recv(SOCKET s, uint8 * buf, uint32 len) {
	...
	if (mr & MR_FS)
		pack_size =
				((((pack_size << 8) & 0xFF00)) | ((pack_size >> 8) & 0x00FF));

#ifdef __DEF_IINCHIP_DBG__   
	alt_printf("%x:pack_size=%x\r\n", s, pack_size);
#endif
...
uint32 sendto(SOCKET s, uint8 * buf, uint32 len, uint8 * addr, uint16 port) {
...
#ifdef __DEF_IINCHIP_DBG__
	alt_printf("%x : sendto():%x.%x.%x.%x(%x), len=%x\r\n", s, addr[0], addr[1],
			addr[2], addr[3], port, len);
#endif

	if (((addr[0] == 0x00) && (addr[1] == 0x00) && (addr[2] == 0x00)
			&& (addr[3] == 0x00)) || ((port == 0x00)) || (len == 0)) {
#ifdef __DEF_IINCHIP_DBG__
		alt_printf("%x : Fail[%x.%x.%x.%x, %.d, %x]\r\n", s, addr[0], addr[1],
				addr[2], addr[3], port, len);
#endif
		return 0;
	}
	...
	while (!((isr = getSn_IR(s)) & Sn_IR_SENDOK)) // wait SEND command completion
	{
		...
		if ((status == SOCK_CLOSED) || (isr & Sn_IR_TIMEOUT)) // Sn_IR_TIMEOUT causes the decrement of Sn_TX_FSR
				{ // -----------------------------------------------
#ifdef __DEF_IINCHIP_DBG__
			alt_printf("%x: send fail.status=0x%02x,isr=%02x\r\n", status, isr);
#endif
			setSn_IR(s, Sn_IR_TIMEOUT);
			return 0;
		}
...
uint32 recvfrom(SOCKET s, uint8 * buf, uint32 len, uint8 * addr, uint16 *port) {
	...
	if (len > 0) {
		...
		case Sn_MR_UDP: // UDP mode 
			...
			{
				head[0] = ((((head[0] << 8) & 0xFF00))
						| ((head[0] >> 8) & 0x00FF));
				head[1] = ((((head[1] << 8) & 0xFF00))
						| ((head[1] >> 8) & 0x00FF));
				head[2] = ((((head[2] << 8) & 0xFF00))
						| ((head[2] >> 8) & 0x00FF));
				head[3] = ((((head[3] << 8) & 0xFF00))
						| ((head[3] >> 8) & 0x00FF));
			}
...
#ifdef __DEF_IINCHIP_DBG__
			alt_printf("UDP msg arrived:%x(0x%04x)\r\n", data_len, data_len);
			alt_printf("source Port : %x\r\n", *port);

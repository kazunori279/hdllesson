...
int main() {
	...
	uint8 * data_buf = (uint8 *) SDRAM_0_BASE + 0x00700000; // buffer for loopack data
	...
	uint8 serverip[4] = { 192, 168, 1, 250 }; // "TCP SERVER" IP address for loopback_tcpc()

	data_buf = (uint8*) SDRAM_0_BASE + 0x00700000;
	...
	alt_printf("SHAR : %x:%x:%x:%x:%x:%x\r\n", mac[0], mac[1], mac[2], mac[3],
			mac[4], mac[5]);
	alt_printf("GWR  : %x.%x.%x.%x\r\n", gw[0], gw[1], gw[2], gw[3]);
	alt_printf("SUBR : %x.%x.%x.%x\r\n", sn[0], sn[1], sn[2], sn[3]);
	alt_printf("SIPR : %x.%x.%x.%x\r\n", ip[0], ip[1], ip[2], ip[3]);
	...
	while (1) {
		...
		// loopback_tcpc(6,serverip, 5000, data_buf,0);
...
void loopback_tcps(SOCKET s, uint16 port, uint8* buf, uint16 mode) {
	...
	uint8 status = getSn_SSR(s);
	switch (status) // check SOCKET status
	{				// ------------
	case SOCK_ESTABLISHED: // ESTABLISHED?
		if (getSn_IR(s) & Sn_IR_CON) // check Sn_IR_CON bit
		{
			alt_printf("%x : Connect OK\r\n", s);
	...
	case SOCK_CLOSE_WAIT: // PASSIVE CLOSED
		...
		alt_printf("%x : LOOPBACK_TCPS(%x) Disconnected.\r\n", s, port);
		break;
		// --------------
	case SOCK_CLOSED: // CLOSED
		close(s); // close the SOCKET
		alt_printf("%x : LOOPBACK_TCPS(%x) Closed.\r\n", s, port);
	...
	case SOCK_INIT: // The SOCKET opened with TCP mode
		...
		alt_printf("%x : LOOPBACK_TCPS(%x) Started.\r\n", s, port);
		break;
	default:
		break;
	}
...
void loopback_tcpc(SOCKET s, uint8* addr, uint16 port, uint8* buf, uint16 mode) {
	...
	case SOCK_INIT: // The SOCKET opened with TCP mode
		...
		alt_printf("%x : LOOPBACK_TCPC(%x.%x.%x.%x:%x) Started.\r\n", s,
	...
	case SOCK_UDP: //
		if ((len = getSn_RX_RSR(s)) > 0) // check the size of received data
			...
			if (len != sendto(s, buf, len, destip, destport)) // send the data to the destination
					{
				alt_printf("%x : Sendto Fail.len=%x,", s, len);
